...
///All the libraries are called here
...
///This is the public class where all the activities will be performed
public class TrackerActivity extends AppCompatActivity {

    private static final int PERMISSIONS_REQUEST = 1;
    private static String[] PERMISSIONS_REQUIRED = new String[]{
            Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.WRITE_EXTERNAL_STORAGE};
	///first declare the buttons
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // String transportID = mTransportIdEditText.getText().toString();
        ...
        String phone = mPrefs.getString(getString(R.string.phone), "");

		///setting current buttons if previously fetched existed
        mTransportIdEditText.setText(transportID);
        mEmailEditText.setText(email);
        mPasswordEditText.setText(password);
        mPhoneEditText.setText(phone);
        spinner1.setSelection(getIndex(spinner1, mPrefs.getString(getString(R.string.branch_prompt), "")));

        if (isServiceRunning(TrackerService.class)) {
            // If service already running, simply update UI.

            setTrackingStatus(R.string.tracking);
        } else if (transportID.length() > 0 && email.length() > 0 && password.length() > 0 && phone.length() >0) {
            ...
            mPhoneEditText.setText(getString(R.string.phone));

    }
	///getting index of selected index via string
	...
	///listens to spinner to set the selection
    ...
    private void setTrackingStatus(int status) {
        ...
        mPhoneEditText.setEnabled(!tracking);
        spinner1.setEnabled(!tracking);
    ...
    private void checkInputFields() {
        if (mTransportIdEditText.length() == 0 || mEmailEditText.length() == 0 ||
                mPasswordEditText.length() == 0 || mPhoneEditText.length() !=11 ) {
            ...
            editor.putString(getString(R.string.phone), mPhoneEditText.getText().toString());
            editor.putString(getString(R.string.branch_prompt), spinner1.getSelectedItem().toString());
            editor.apply();
