...
///Public class of tracker service
public class TrackerService extends Service implements LocationListener {
	...
	///builds firebase settings and gets all the relevant strings
    @Override
	...
	///This person is basically responsible for authentication of provided email and password from firebase
    ...
    private void loadPreviousStatuses() {
		///trasportid has the the name,number and branch concatenated
        String transportId = mPrefs.getString(getString(R.string.transport_id), "")+ ','+mPrefs.getString(getString(R.string.phone), "")+','+mPrefs.getString(getString(R.string.branch_prompt), "");
        ...
        mFirebaseTransportRef.addListenerForSingleValueEvent(new ValueEventListener() {
			///Works on data snapshots from firebase and calls locating function
            @Override
	...
	///creating api client that is responsible for sending receiving locations either from wifi or data (fused location) 
    private GoogleApiClient.ConnectionCallbacks mLocationRequestCallback = new GoogleApiClient
            .ConnectionCallbacks() {
		///creating a callback which stores values in location request which will be used with googleapi
        @Override
        public void onConnected(Bundle bundle) {
            LocationRequest request = new LocationRequest();
			request.setInterval(30000); ///30 second delay
            request.setFastestInterval(30000); ///30 second delay if location is available 
            request.setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY );///Accurate locations 
            request.setSmallestDisplacement(30.0f);///Parameter of 30 meter
            LocationServices.FusedLocationApi.requestLocationUpdates(mGoogleApiClient,
                    request, TrackerService.this);//getting the locations
	...
	///This is where we are sending information to firebase after getting last location.
    @Override
        ...
        new java.util.Timer().schedule(
                new java.util.TimerTask() {
                    ...
                    public void run(Location location) {
						////This list as lat,lng,current time and power(of battery)
                        ...
                        mFirebaseTransportRef.push().setValue(mTransportStatuses.getFirst());
                        //  }
